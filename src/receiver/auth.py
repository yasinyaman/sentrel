...
class DSNAuth:
    ...
    def __init__(
        ...
        Args:
            allowed_keys: List of allowed public keys.
            auth_required: If True, authentication is required (deny if no keys configured).
        """
    ...
    def validate_key(self, public_key: Optional[str]) -> bool:
        ...
        # If no allowed keys configured
        if not self.allowed_keys:
            # In production (auth_required=True), deny all if no keys configured
            # This prevents accidental open access
            if self.auth_required:
                return False
            # In development (auth_required=False), allow all
            return True
